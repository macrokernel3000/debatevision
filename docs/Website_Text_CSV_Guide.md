...
手機首頁、底部選單、模式卡、數量標題、卡組標籤與手機流程教學可以在這裡修改：
...
`手機介面文字.csv` 集中管理手機版專屬文字，欄位如下：

```text
文字ID,玩法ID,區域,內容,說明
```

- `文字ID`：程式使用的固定代號，請勿任意修改。
- `玩法ID`：`global` 代表所有活動共用；其他值對應特定玩法。
- `區域`：方便篩選，例如手機首頁、模式選擇、數量控制、卡組設定。
- `內容`：實際顯示在手機畫面上的文字，主要編輯這一欄。
- `說明`：這段文字的用途。

目前涵蓋異境求生、現實召喚、誰更重要、銷售密令、隱喻羅盤、推理解密、卡片字典，以及手機首頁、底部選單與卡組編輯器。

修改後雙擊 `網站更新.command`，網站會把內容更新到 `data/generated/ui-texts.js`。
