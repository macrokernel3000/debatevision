...
3. 如果使用新牌組或新分類，確認 `data/cards/*.csv` 欄位完整。
4. 如果需要新小圖示，放到 `assets/icons/{deck_id}/`，CSV 的 `卡牌圖示` 只填 icon id。
5. 如果需要大圖，放到適合的素材資料夾，CSV 的 `圖片` 欄只填 image id 或檔名。
6. 執行 `網站更新.command` 或 `node scripts/build-lexicons.mjs`。
7. 檢查 `data/generated/` 已更新。
...
- `卡牌圖示`：小圖示，通常顯示在抽選池、候選格、列表。填 icon id，例如 `world_icon_zombie`。
